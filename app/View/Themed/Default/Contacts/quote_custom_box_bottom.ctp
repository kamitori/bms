<?php
	...
	$sum_tax = 0;
	$sum_amount = 0;
	//pr($arr_quotation);die();
	foreach($arr_quotation as $key=>$quote){
        $sum_sub_total      +=   $arr_quotation[$key]['sum_sub_total'] ;
        $sum_amount     +=   $arr_quotation[$key]['sum_amount'];
        $sum_tax        +=   $arr_quotation[$key]['sum_tax'] ;
	}

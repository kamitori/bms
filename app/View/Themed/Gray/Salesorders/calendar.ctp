...
<div id="calendar-left">
    <ul id="have_data" class="list_day_rs">
        ...
        foreach ($arr_responsible_task as $key => $value) { ?>
            <li id="li<?php echo $key; ?>" onclick="<?php echo $controller; ?>_calendar_onchange_rep(this, '<?php echo $key; ?>', '<?php echo $type ?>')">
                <?php
                    if($type=='contacts')
...
$(function(){
    ...
    scheduler.templates.tooltip_text = function(b, d, c) {
        ...
        str = str + "<br/><b>Contact:</b> " + c.text_contact_name;

        str = str + "<br><b>(SO) Our Rep:</b>" + c.text_so_rep;
        str = str + "<br><b>(SO) Our CSR: </b>" + c.text_so_csr;
    ...
    scheduler.load("<?php echo URL; ?>/<?php echo $controller; ?>/calendar_json/<?php echo $type; ?>/" + get_date[0] + "/" + get_date[1]);
    ...
    scheduler.templates.event_header = function(start,end,ev){
        // return ev.name;
        return ev.text_so.replace("_1_","&");

    };

    scheduler.templates.event_text = function(start,end,ev){
        // return "";
        return ev.text_responsible;//salesorder_heading;
    };

    scheduler.templates.event_bar_text = function(a, b, c) {
        return c.text_so.replace("_1_","&"); //salesorder_heading; // for multi days
    ...
    scheduler.filter_month = scheduler.filter_day = scheduler.filter_week = function(id, event) {
        ...
        if ( (filters_status[event.status] || event.status==scheduler.undefined) && (filters_responsible_task[event.rep_id] || event.rep_id==scheduler.undefined) )
        {
            return true;
        }
        return false;
    ...
    scheduler.attachEvent("onDblClick", function (event_id, native_event_object){
        location.href = '<?php echo URL; ?>/tasks/redirect_so/' + event_id;
    });

    <div class="tab_1 full_width">
        <span class="title_block bo_ra1">
            <span class="fl_dent">
                <h4><?php echo translate('Tasks relating to this enquiry'); ?></h4>
            </span>
            <?php if( $this->Common->check_permission('tasks_@_entry_@_add', $arr_permission) ){ ?>
            <a href="<?php echo URL; ?>/enquiries/tasks_add/<?php echo $enquiry_id; ?>" title="Add new task">
        ...
        <div class="container_same_category" style="height:176px;overflow-y:auto;">
            ...
            <ul class="ul_mag clear bg<?php echo $i; ?>" id="Salesorder_Job_<?php echo $value['_id']; ?>">
                ...
                <li class="hg_padd" style="width:14%"><?php if(isset($value['type']))echo $value['type']; ?></li>
                <li class="hg_padd line_mg" style="width:10%"><?php if(isset($value['our_rep']))echo $value['our_rep']; ?></li>
                <li class="hg_padd line_mg center_txt" style="width:10%"><?php echo (isset($value['work_start']) ? $this->Common->format_date($value['work_start']->sec):''); ?></li>
                <li class="hg_padd line_mg center_txt" style="width:10%"><?php echo (isset($value['work_end']) ? $this->Common->format_date($value['work_end']->sec):''); ?></li>
                <li class="hg_padd" style="width:10%;"><?php if(isset($value['status']))echo $value['status']; ?></li>
                <?php if( $this->Common->check_permission('tasks_@_entry_@_delete', $arr_permission) ){ ?>
                <li class="hg_padd bor_mt" style="width:1.5%">
                    <div class="middle_check">
                        <a title="Delete link" href="javascript:void(0)" onclick="resource_remove_task_enquiry('<?php echo $value['_id']; ?>')">
...
function resource_remove_task_enquiry(task_id){
    confirms( "Message", "Are you sure you want to delete?",
        function(){
            $.ajax({
                 url: '<?php echo URL; ?>/enquiries/tasks_delete/' + task_id,
                 timeout: 15000,
                 success: function(html){
                     if(html == "ok"){
                         $("#Salesorder_Job_" + task_id).fadeOut();
                     }else{
                         alerts("Error: ", html);
                     }
                 }

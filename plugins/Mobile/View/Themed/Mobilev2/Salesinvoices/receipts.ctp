    ...
    <li id="list-<?php echo $value['_id']; ?>" class="ui-li-static ui-body-inherit"  data-role="collapsible" data-role="collapsible" data-collapsed-icon="arrow-d" data-expanded-icon="arrow-u" data-iconpos="right" data-inset="false">
        <h2>
            <div class="ui-block-a" style="width:30%">
                <a class="link-to-entry" href="<?php echo M_URL; ?>/receipts/entry/<?php echo $value['_id']; ?>"><?php echo $value['code']; ?></a>
            </div>
            <div class="ui-block-b" style="width:60%"><?php echo $value['code'];?></div>
        </h2>
        <ul data-role="listview" data-theme="b">
            ...
            <li>
                <div class="ui-block-a" style="width:30%"><b>Write off</b></div>
                <div class="ui-block-b" style="width:70%">
                    <?php if (isset($value['receipts'][0]['write_off'])&& $value['receipts'][0]['write_off']) echo 'Yes'; else echo 'No'; ?>
                </div>
            </li>
            <li>
                <div class="ui-block-a" style="width:30%"><b>Amount</b></div>
                <div class="ui-block-b ui-checkbox" style="width:70%">
                    <?php echo number_format($value['receipts'][0]['amount'],2); ?>
                </div>
            </li>
            <li>
...
$(function(){
    ...
    $('#bt_add_receipt').click(function(){
            $.ajax({
                url:"<?php echo URL;?>/salesinvoices/receipt_add/" + "<?php echo $salesinvoice_id ?>",
                timeout: 15000,
    ...
    $("#list-record").text('<?php echo number_format($total_amount,2); ?>').attr(attrObj).css("text-align","right").attr("id","sum-amount");
    $("#bt_add_receipt").attr("class","ui-state-disabled ui-link ui-btn");
})
</script>
...
<form id="form_salesinvoice" action="<?php echo URL;?>/salesinvoices/customer_report/detail" method="POST">
    <div class="tab_2_inner">
        ...
        <div class="width_in float_left indent_input_tp" style="width:61.5%">
            ...
            <script type="text/javascript">
                    $(function () {
                        $("#status").combobox(<?php echo json_encode($arr_data['salesinvoices_status']); ?>);
                    });
            </script>    
        </div>
        <p></p>
        ...
        <div class="width_in float_left indent_input_tp" style="width:61.5%">
             <?php echo $this->Form->input('type', array(
                ...
                <script type="text/javascript">
                    $(function () {
                        $("#type").combobox(<?php echo json_encode($arr_data['salesinvoices_type']);
                        ?>);
                    });
                </script>  
        </div>
        ...
        <div class="indent_new width_in float_left" style="width:61%">
            <?php
                ...
                <input id="taxId" name="tax_id" value="" type="hidden" />
                 <script type="text/javascript">
                    $(function () {
                        $("#tax").combobox(<?php echo json_encode($arr_data['salesinvoices_tax']); ?>);
                    });
            </script>
        </div>
        <p></p>
        <p class="clear">
            <span class="label_1 float_left minw_lab2" style="height:50%">Employee</span>
        </p>
        <div class="indent_new width_in float_left" style="width:61%">
            <?php
            ...
            <input type="hidden" value="" id="employee" name="employee_id">
            <script type="text/javascript">
                $(function(){
                    window_popup('contacts', 'Specify Employee','employee','click_open_window_contactsemployee');
                });
            </script>
        </div>
...
<script type="text/javascript">
    $(function(){
        ...
        $('#CancelButton').click(function(){
            window.location.replace('<?php echo URL; ?>/salesinvoices/options');
        });
        $('#ContinueButton').click(function(){
            ...
            else
            {
                ...
                $.ajax({
                    url: '<?php echo URL; ?>/salesinvoices/check_exist_customer/',
                    type: 'POST',
                    data: {data : $('#form_salesinvoices').serialize()},
                    async: false,
                    success : function(result)
                    {
                        if(result=='empty')
                        {
                          ...
                          $('#form_salesinvoices :input').each(function(index, el) {
                              $(this).val('');
                          });
                          $('#report_type').val('detailed');
                    }
                });                
                return false;*/
                $.ajax({
                    url: '<?php echo URL; ?>/salesinvoices/customer_report',
                    type: 'POST',
                    data:  $('input','#form_salesinvoice').serialize(),
                    success : function(result) {
                        if(result=='empty')
                            alerts('Message','No record!');
                        else
                           $("#form_salesinvoice").submit();
                    }
                 });
            }
        });

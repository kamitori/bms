<?php echo $this->Form->create('Purchaseorder', array('id' => 'purchaseorders_popup_form' . $key)); ?>
            ...
            <span class="block_sear  block1">
                ...
                <div class="box_inner_search float_left">
                    ...
                    <div class="styled_select2 float_left">
                        <?php
                        echo $this->Form->input('Purchaseorder.company', array(
...
<div class="block_dent2 container_same_category" style="overflow: auto;overflow-x: hidden;max-width:1000px; margin: 0 auto; height:430px;" id="list_view_purchaseorders">
    <table class="jt_tb" id="Form_add" style="font-size:12px;">
        ...
        <tbody>
            ...
            <?php
            $i = 0; $STT = 0;
            foreach ($arr_purchaseorder as $value) {
                $i = 1 - $i; $STT += 1;
                ?>
                <tr class="jt_line_<?php if ($i == 1) { ?>black<?php } else { ?>light<?php } ?>" onclick="after_choose_purchaseorders<?php if (substr($key, 0, 1) == '_') echo $key; ?>('<?php echo $value['_id']; ?>', '', '<?php echo $key; ?>')">
                    ...
                    <td align="center"><?php if(isset($value['purchord_date']))echo $this->Common->format_date( $value['purchord_date']->sec, false); ?>
                        <input type="hidden" id="after_choose_purchaseorders<?php echo $key; ?><?php echo $value['_id']; ?>" value="<?php echo htmlentities(json_encode($value)); ?>">
                    </td>
                </tr>
            <?php } ?>

    ...
    <table class="jt_tb" id="Form_add" style="font-size:12px;">
        ...
        <tbody>
            ...
            foreach ($arr_salesinvoices as $value) {
                $i = 1 - $i; $STT += 1;
                ?>
                <tr class="jt_line_<?php if ($i == 1) { ?>black<?php } else { ?>light<?php } ?>" onclick="after_choose_salesinvoices<?php if (substr($key, 0, 1) == '_') echo $key; ?>('<?php echo $value['_id']; ?>', '', '<?php echo $key; ?>')">
                    ...
                    <td align="center"><?php if(isset($value['invoice_date']))echo $this->Common->format_date( $value['invoice_date']->sec, false); ?>
                        <input type="hidden" id="after_choose_salesinvoices<?php echo $key; ?><?php echo $value['_id']; ?>" value="<?php echo htmlentities(json_encode($value)); ?>">
                    </td>
                </tr>
            <?php } ?>

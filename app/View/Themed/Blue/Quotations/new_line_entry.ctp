...
$(document).ready(function() {
	...
	$(".icon_emp4").click(function(){
		...
		window.location.assign("<?php echo URL;?>/<?php echo $controller; ?>/rfqs_list/"+ids);
	});
	...
	$('input[type!=hidden]','#container_products').change(function(){
		...
		//Kiem tra va thuc hien tinh toan in neu la printer
		if($("#is_printer_"+ids).val()==1 && names!='custom_unit_price'){
			...
			cal_line_entry(values,names,function(result){
				...
				if( $.inArray($("#quotation_status").val(), ['In progress', 'Rejected', 'Cancelled']) == -1 ){
					$("#quotation_status").val('In progress').trigger("change");
				}
				//SELF==================================================
...
function cal_price_printing(data,fieldchange,callBack){
	$.ajax({
		url : "<?php echo URL.'/'.$controller;?>/cal_price_printing",
		type:"POST",
		data: {data:JSON.stringify(data),fieldchange:fieldchange},
		success: function(result){
			if(typeof callBack == "function")

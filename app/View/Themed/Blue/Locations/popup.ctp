...
<div style="float: left;">
    ...
    <div class="float_left">
        <h6 class="float_left" style="margin-top:2px"><?php echo translate('Filter by') . ': ' . translate('Location'); ?> &nbsp;</h6>
    ...
    <table class="jt_tb" id="Form_add" style="font-size:12px;">
        <thead style="position: fixed;" id="pagination_sort">
            <tr>
                <th style="width:60px"><?php echo translate('Ref no'); ?><span id="sort_no" rel="no" class="desc"></span></th>
                 <th style="width:248px"><?php echo translate('Location name'); ?><span id="sort_name" rel="name" class="desc"></span></th>
                 <th style="width:272px"><?php echo translate('Company name'); ?><span id="sort_company" rel="company" class="desc"></span></th>
                <th style="width:100px"><?php echo translate('Contact name'); ?><span id="sort_contact_name" rel="contact_name" class="desc"></span></th>
        ...
        <tbody>
            ...
            foreach ($arr_location as $value) {
                $i = 1 - $i; $STT += 1;
                ?>
                <tr class="jt_line_<?php if ($i == 1) { ?>black<?php } else { ?>light<?php } ?>" onclick="after_choose_locations<?php if (substr($key, 0, 1) == '_') echo $key; ?>('<?php echo $value['_id']; ?>', '', '<?php echo $key; ?>')">

                    <td align="center"><?php if(isset($value['code']))echo $value['code']; ?>&nbsp;</td>
                    <td align="left"><?php if(isset($value['name']))echo $value['name']; ?>&nbsp;</td>
                    <td align="left"><?php if(isset($value['company_name']))echo $value['company_name']; ?>&nbsp;</td>
                    ...
                    <td align="center"><?php if(isset($value['date']))echo $this->Common->format_date( $value['date']->sec, false); ?>
                        <input type="hidden" id="after_choose_locations<?php echo $key; ?><?php echo $value['_id']; ?>" value="<?php echo htmlentities(json_encode($value)); ?>">
                    </td>
                </tr>
            <?php } ?>

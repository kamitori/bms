...
?>
    <li id="list-<?php echo $key ?>" class="ui-li-static ui-body-inherit"  data-role="collapsible" data-role="collapsible" data-collapsed-icon="arrow-d" data-expanded-icon="arrow-u" data-iconpos="right" data-inset="false">
        <h2>
            ...
            <div class="ui-block-b" style="width:60%"><?php if (isset($value['name'])) echo isset($options['contacts_addresses_name'][$value['name']]) ? $options['contacts_addresses_name'][$value['name']] : '' ?></div>
        </h2>
...
$(function(){
    $("#delete_button").click(function(){
        ...
        saveOption({opname: "addresses", data: {"deleted" : 1}, key : key, controller:'contacts', callBack: function(){
    ...
    $("#list-view").on("change","input,select",function(){
        ...
        loading("Saving...");
        saveOption({opname: "addresses", data: data, key : ids, controller:'contacts', callBack: function(){
                if(names == "name" ){
                    var text = $("#name_"+ids+" option:selected").text();
                    $(".ui-block-b:first","#list-"+ids).html(text);
                } else if(names == "default" && inval == 1) {
                    $("input[name='default']:checked","#list-view").prev().removeClass("ui-checkbox-on").addClass("ui-checkbox-off");// loai bo check_box dang duoc checked khac
                    $("input[name='default']:checked","#list-view").prop("checked",false);// loai bo check_box dang duoc checked khac
                    $("#default_"+ids).prop("checked",true);
                    $("#default_"+ids).prev().removeClass("ui-checkbox-off").addClass("ui-checkbox-on");
                    addresses_auto_save(ids);
                }
                $.mobile.loading( 'hide' );
            }
        });
...
function addresses_auto_save(ids){
    $.ajax({
        url:"<?php echo M_URL.'/'.$controller; ?>/addresses_default_key",
        type:"POST",
        data: {  Contact :  
                           { addresses_default_key: ids }
              },  

...
function general_details_save(object,content){ 
   $("#block_full_bookings1 input, select").change(function() {
       var fieldname = $(this).attr("name");
       var values = $(this).val();
       var ids = $("#mongo_id").val();
       var fieldtype = $(this).attr("type");
...
<?php if(!$this->Common->check_permission('locations_@_entry_@_edit',$arr_permission)){?>
	// $('.icon_emp').remove();

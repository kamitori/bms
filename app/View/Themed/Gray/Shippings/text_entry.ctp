...
$(document).ready(function() {
	...
	$("textarea, .rowedit input").focus(function(){
	...
	$("textarea, .rowedit input").change(function(){
		...
		if(names=='shipped')
		{
			...
			{
				alerts('Message','The shipped quantity cannot be greater than the original quantity.');
				$(this).val($(this).attr('oldValue'));
				return false;
			}
		}
		...
		{
			var shipped = $('#shipped_'+ids).val();
			if(shipped!=undefined||shipped!='')
			{
				if(parseInt(inval)<parseInt(shipped))
				{
...
<?php } else {?>
	$("input,select,textarea", "#block_full_products").each(function() {
		$(this).attr("disabled", true).css("background-color", "transparent");
	});
	$("#bt_add_products").remove();
	$("#load_subtab").find("a").each(function(){
        $(this).remove();
    });
    $(".icon_emp","#mCSB_1").remove();
<?php } ?>

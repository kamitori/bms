            ...
            <span class="block_sear  block1" style=" ">
                ...
                <div class="box_inner_search float_left" id="box_products_company_name">
                    ...
                    <div class="styled_select2 float_left" style=" ;">
                        ...
                        ?>
                        <a href="javascript:void(0)" onclick="$('#window_popup_salesorders_Company_<?php echo $key; ?>').val(''); $('#<?php echo $controller; ?>_popup_submit_subtton_<?php echo $key; ?>').click();">
        ...
        <span class="float_left">
            <span class="block_sear  block1">
                ...
                <div class="box_inner_search float_left">
                    <a href="javascript:void(0)" onclick="$('#<?php echo $controller; ?>_popup_submit_subtton_<?php echo $key; ?>').click();"><span class="icon_search"></span></a>
                        <div class="styled_select2 float_left">
                        	<select name="data[Salesorder][status]" style="background: #636363;color: #fff;margin-left: -2px;margin-top:-2px;width: 100px;" onchange="$('#window_popup_<?php echo $controller; ?>_Company_<?php echo $key; ?>').val('');$('#<?php echo $controller; ?>_popup_submit_subtton_<?php echo $key; ?>').click();" >
                        		<option value=""></option>
    	                		<?php
    	                			$status = (isset($status) ? $status : '');
    	                			foreach($arr_order_status as $op_key=>$value)
...
// Ẩn nút submit này
 ...
 <ul class="menu_control float_right">
    <li >
        ...
        <?php if($key=="create_invoice_from_combine_salesorders"){ ?>
            <a href="javascript:void(0)" id="submit_iv_from_so<?php echo $key ?>">Combine and create invoice</a>
        <?php }else{ ?>
...
<div class="block_dent2 container_same_category" style="overflow: auto;overflow-x: hidden;max-width:1000px; margin: 0 auto; height:430px;" id="list_view_salesinvoices">
    <table class="jt_tb" id="combine_salesorders<?php echo $key ?>" style="font-size:12px;">
        <thead style="position: fixed;" id="pagination_sort">
            <tr>
            	<th style="width:20px"><input type="checkbox" onchange="checkAllSO(this)" /></th>
                <th style="width:90px"><?php echo translate('Ref no'); ?><span id="sort_code" rel="code" class="desc"></span></th>
                <th style="width:130px"><?php echo translate('Date'); ?><span id="sort_salesorder_date" rel="salesorder_date" class="desc"></span></th>
                <th style="width:80px"><?php echo translate('Job'); ?><span id="sort_job_number" rel="job_number" class="desc"></span></th>
                <th style="width:350px"><?php echo translate('Heading'); ?><span id="sort_heading" rel="heading" class="desc"></span></th>
                <th style="width: 130px"><?php echo translate('Status'); ?><span id="sort_status" rel="status" class="desc"></span></th>
            </tr>
        </thead>
        <tbody>
            <tr>
            ...
            <?php
            $i = 0; $STT = 0;
            foreach ($arr_salesorders as $value) {
                $i = 1 - $i; $STT += 1;
                ?>
                <tr class="jt_line_<?php if ($i == 1) { ?>black<?php } else { ?>light<?php } ?>" id="tr_<?php echo $value['_id']; ?>">
                    <td align="center"><input class="check_salesorders" name="data[combine_salesorders][<?php echo $value['_id'] ?>]" type="checkbox" id="<?php echo $value['_id'] ?>" /></td>
                    <td align="center" onclick="$('input[type=checkbox]',$(this).parent()).click()"><?php if(isset($value['code']))echo $value['code']; ?>&nbsp;</td>
                    <td align="center" onclick="$('input[type=checkbox]',$(this).parent()).click()"><?php if(isset($value['salesorder_date']))echo $this->Common->format_date($value['salesorder_date']->sec); ?>&nbsp;</td>
                    <td align="left" onclick="$('input[type=checkbox]',$(this).parent()).click()"><?php if(isset($value['job_number']))echo $value['job_number']; ?>&nbsp;</td>
                    <td align="left" onclick="$('input[type=checkbox]',$(this).parent()).click()"><?php if(isset($value['heading']))echo $value['heading']; ?>&nbsp;</td>
                    <td align="left" onclick="$('input[type=checkbox]',$(this).parent()).click()"><?php if(isset($value['status']))echo $value['status']; ?>&nbsp;</td>
                </tr>
            <?php } ?>
...
$(function(){
	var store = localStorage['combine_salesorder<?php echo $key ?>'];
	if(store!=undefined){
		...
		$("tbody tr:first","#combine_salesorders<?php echo $key ?>").after(html);
		for(i in data){
			$("#tr_"+i).removeClass();
			$("#"+i).attr("checked",true);
		}
	...
	$(".check_salesorders","#combine_salesorders<?php echo $key ?>").change(function(){
		var data = localStorage['combine_salesorder<?php echo $key ?>'];
		if(data == undefined)
			data = {};
		else
		...
		if($(this).is(":checked"))
			...
			delete(data[id]);
		localStorage['combine_salesorder<?php echo $key ?>'] = JSON.stringify(data);
	});
    $("#submit_batch<?php echo $key ?>").click(function(){
        $("#window_popup_salesorders<?php echo $key ?>").data("kendoWindow").close();
        var data = $("input:checked","#combine_salesorders<?php echo $key ?>").serialize();
        $.ajax({
            url: "<?php echo URL.'/jobs/combine_orders' ?>",
            data: data,
            type: "POST",
            success: function(result){
                if( result == "ok"){
                    localStorage.removeItem('combine_salesorder<?php echo $key ?>');
                    window.location.assign("<?php echo URL.'/salesorders/entry/'; ?>");
                } else
                    alerts("Message",result);
            }
        });
    });
    $("#submit_iv_from_so<?php echo $key ?>").click(function(){
        $("#window_popup_salesorders<?php echo $key ?>").data("kendoWindow").close();
        var data = $("input:checked","#combine_salesorders<?php echo $key ?>").serialize();
        $.ajax({
            url: "<?php echo URL.'/jobs/combine_orders' ?>",
            ...
            success: function(result){
                if( result.status == "ok"){
                    localStorage.removeItem('combine_salesorder<?php echo $key ?>');
                    create_full_salesinvoice(result.saleorder_id);
                } else
                    alerts("Message",result);
            }
        });
    ...
    //reset all saleorder checkbox
    $('.check_salesorders', "#combine_salesorders<?php echo $key ?>").prop('checked', false);
    $('.check_salesorders', "#combine_salesorders<?php echo $key ?>").trigger("change");
});
function checkAllSO(obj)
{
    if($(obj).is(':checked'))
    {
        //check all
        $('.check_salesorders', "#combine_salesorders<?php echo $key ?>").prop('checked', true);
    }
    else
    {
        //uncheck all

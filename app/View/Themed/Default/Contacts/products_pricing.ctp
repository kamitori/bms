...
<div class="clear_percent">
	...
	<div class="float_left" style=" width:27%;margin-left:1%;float: left;" id="company_product_price_break">
		<?php echo $this->element('..'.DS.'Contacts'.DS.'products_price_break'); ?>
	</div>
	...
	<div class="float_left " style=" width:44%;margin-left:1%; float: left;">
		<div class="tab_1 full_width">
			...
			<div id="company_products_pricing_note" style="height: 483px;">
				<textarea class="area_t3" onchange="contact_product_save_notes(this)" style="height:98%;"><?php echo $arr_product['notes']; ?></textarea>
				<script type="text/javascript">
				function contact_product_save_notes(object){
					$.ajax({
						url: "<?php echo URL; ?>/contacts/products_save_notes/<?php echo $contact_id; ?>/<?php echo $key; ?>",
						timeout: 15000,
						type: "POST",
...
function contact_pricebreak_delete(key){
	confirms( "Message", "Are you sure you want to delete?",
		function(){
			$.ajax({
				 url: '<?php echo URL; ?>/contacts/products_pricebreak_delete/<?php echo $contact_id; ?>/<?php echo $key; ?>/' + key,
				 timeout: 15000,
				 success: function(html){
					 if(html == "ok"){
						$("#company_product_price_break_" + key).remove();
					 }else{
						alerts("Error: ", html);
					 }
				 }
...
function contact_product_pricebreak_save(object){
		$.ajax({
			url: "<?php echo URL; ?>/contacts/product_pricebreak_save/<?php echo $contact_id; ?>/<?php echo $key; ?>/" + $(object).attr("rel"),
			timeout: 15000,
			type: "POST",

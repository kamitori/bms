    ...
    <li id="list-<?php echo $value['_id'] ?>" class="ui-li-static ui-body-inherit"  data-role="collapsible" data-role="collapsible" data-collapsed-icon="arrow-d" data-expanded-icon="arrow-u" data-iconpos="right" data-inset="false">
        <h2>
            <div class="ui-block-a" style="width:30%">
                <a class="link-to-entry" href="<?php echo M_URL; ?>/contacts/products_pricing/<?php echo $value['_id']; ?>"><?php echo $value['code']; ?></a></div>
...
$(function(){
    ...
    $("#delete_button").click(function(){
        ...
        saveOption({opname : "pricing", key : ids, data : {deleted : "true"}, controller : "contacts", callBack : function(){
...
function after_choose_products(product_id,product_name){
    ...
    saveOption({opname : "pricing", key : null, data : data, controller : "contacts", callBack : function(result){
        result = $.parseJSON(result);
        for(var i in result["pricing"]){
            break;
        }
        window.location.assign("<?php echo M_URL.'/contacts/products_pricing/' ?>"+i);
    }});
}

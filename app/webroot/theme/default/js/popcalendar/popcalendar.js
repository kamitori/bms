...
var popCalendar = {
  ...
  construct : function() {
	...
	for (var datePointer=1; datePointer <= numDaysInMonth; datePointer++) {
	  ...
	  if (!this.enablePast &&
		...
		// namnb add get_date_main_js 19.06.2011
		selDayAction = 'href="javascript:void(0);" onclick="javascript:popCalendar.close('+datePointer+');get_date_main_js('+datePointer+','+ this.monthSelected +', ' +this.yearSelected+ ');"';
	  }
   ...
   */
  show: function(event, btn, ctl, format, past, x, y, start) {
	if (start != null) this.startAt = start;
	if (!format) format = nv_formatString;
	if (!btn) btn = this;

	// if (!ctl) ctl = btn; namnb 2:43 PM 9/17/2010
	ctl = jQuery(btn).prev("div").find("input").attr("id");
	if( ctl == undefined )
	{
		ctl = "input_tmp_to_show_canlendar";
	}
	...
	if (this.bPageLoaded) {
	  if (this.o.style.visibility == 'hidden') {
		...
		if (typeof jQuery != 'undefined') {               // use jQuery if available
		  this.o.style.top = (this.fixedY == -1) ? jQuery(btn).position().top  + btn.offsetHeight + 'px' : this.fixedY + 'px' ;
		  this.o.style.left= (this.fixedX == -1) ? jQuery(btn).position().left + 'px' : this.fixedX + 'px';
		} else {

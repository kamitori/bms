						...
						<div>
							...
							<ul class="find_list">
								<?php foreach ($vv as $kmm => $vmm) { // loop Items    ?>
									...
									<a class="swith_options" <?php if(isset($vmm['codekey'])&&($vmm['codekey']=='print_mini_list'||$vmm['codekey']=='print_invoice'||$vmm['codekey']=='print_sales_orders'||$vmm['codekey']=='print_mini_list_(for_internal_use)'||$vmm['codekey']=='print_sales_order'||$vmm['codekey']=='print_shipping_note')){?> target="_blank" <?php } ?> id="<?php if (isset($vmm['codekey'])) echo $vmm['codekey']; ?>" style="cursor:pointer;line-height:24px;<?php if(isset($vmm['not_finished'])&&$vmm['not_finished']==1) echo 'color:red;'; ?>" <?php if (isset($vmm['url']) && $vmm['url'] != '') echo 'href="' . URL . '/' . $vmm['url'] . '"'; elseif( $vmm['codekey']=='print_mini_list'||$vmm['codekey']=='print_sales_orders'||$vmm['codekey']=='print_mini_list_(for_internal_use)' ) {?>  href="<?php echo URL . '/' . $controller; ?>/view_minilist"            <?php } elseif( $vmm['codekey']=='print_sales_order'|| $vmm['codekey']=='print_invoice'){ ?> href="<?php echo URL . '/' . $controller; ?>/view_pdf" <?php } elseif($vmm['codekey']=='print_shipping_note'){ ?> href="<?php echo URL . '/' . $controller; ?>/view_shipping_note" <?php } else { ?> onclick="swith_options('<?php if (isset($vmm['codekey'])) echo $vmm['codekey']; ?>');"<?php } ?>>

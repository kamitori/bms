<?php
class SalesordersController extends MobileAppController {
	var $modelName = 'Salesorder';
	var $name = 'Salesorders';
	
	/*
	/*	Progress before filter
	...
	public function beforeFilter() {
		...
		if(!$this->request->is('ajax'))
			$this->set('arr_tabs', array(
			           			...
			           			'task' => array('task'),
			           			'ship/Invoice' => array('ship','invoice'),
			           			'document' => array('docs'),
			           			'other'  => array('communications'),
			           			'asset tags' => array('asset_tags'),
			           			'costings' => array('costings'),
			           			));
	}
	...
	public function entry($id = '0', $num_position = -1){
		$arr_tmp = $this->entry_init($id, $num_position, 'Salesorder', 'salesorders');
		$arr_tmp['salesorder_date'] = (is_object($arr_tmp['salesorder_date'])) ? date('m/d/Y', $arr_tmp['salesorder_date']->sec) : '';
		...
		$arr_salesorders_type = $this->Setting->select_option(array('setting_value' => 'sales_order_type'), array('option'));
		$this->set('arr_salesorders_type', $arr_salesorders_type);

		$arr_salesorders_payment_terms = $this->Setting->select_option(array('setting_value' => 'salesinvoices_payment_terms'), array('option'));
		$this->set('arr_salesorders_payment_terms', $arr_salesorders_payment_terms);

		$arr_salesorders_status = $this->Setting->select_option(array('setting_value' => 'salesorders_status'), array('option'));
		$this->set('arr_salesorders_status', $arr_salesorders_status);

		$this->selectModel('Tax');
		$arr_salesorders_tax = $this->Tax->tax_select_list();
		$this->set('arr_salesorders_tax', $arr_salesorders_tax);	
		

		$this->set('arr_delivery_method', $this->Setting->select_option(array('setting_value' => 'salesorder_delivery_method'), array('option')));
		$this->set('arr_country', $this->Setting->select_option(array('setting_value' => 'lists_country'), array('option')));


		$arr_tmp1['Salesorder'] = $arr_tmp;
		$this->data = $arr_tmp1;
		$arr_salesorder_id = array();
		if (isset($arr_tmp['our_rep_id']))
			$arr_salesorder_id[] = $arr_tmp['our_rep_id'];
	...
	public function auto_save(){
		if (!empty($this->data)) {
			$id = $this->get_id();
			$arr_return = array();
			$arr_post_data = $this->data['Salesorder'];
			$arr_save = $arr_post_data;
			$arr_save['_id'] = new MongoId($id);
			$this->selectModel('Salesorder');
		
			if(isset($arr_save['code'])){
				$arr_tmp = $this->Salesorder->select_one(array('code' => (int) $arr_save['code'], '_id' => array('$ne' => new MongoId($id))));
			...
			$arr_tmp = $this->Salesorder->select_one(array('_id' =>  new MongoId($id)),array('status','company_id','customer_po_no'));

			$salesorder_date = $this->Common->strtotime($arr_save['salesorder_date'] . ' 00:00:00');
			$arr_save['salesorder_date'] = new MongoDate($salesorder_date);
			...
			if ($this->Salesorder->save($arr_save))
				...
				$arr_return = array('status'=>'error','message'=>'Error: ' . $this->Salesorder->arr_errors_save[1]);
		...
		if (!$error) {
			...
			if ($this->Salesorder->save($arr_save)) {
				...
				if($this->request->is('ajax'))
					...
					$this->redirect('/mobile/salesorders/entry');
			} else {
	...
	public function add() {
		...
		if ($this->Salesorder->add())
			$this->redirect('/mobile/salesorders/entry/' . $this->Salesorder->mongo_id_after_save);
		die;
	}
	...
	function lists( $content_only = '' ) {
		...
		// dùng cho điều kiện
		$cond = array();
		if( $this->Session->check('salesorders_entry_search_cond') ){
			$cond = $this->Session->read('salesorders_entry_search_cond');
		}
		...
		$this->selectModel('Setting');
		$this->set('arr_salesorders_type', $this->Setting->select_option(array('setting_value' => 'salesorders_type'), array('option')));
	...
	function popup($key = "") {
        ...
        $arr_salesorders = $this->Salesorder->select_all(array(
            ...
            'limit' => $limit,
            'skip' => $skip,
            'arr_field' => array('company_id', 'company_name', 'contact_name','contact_id','name','code','payment_due_date')
        ));
    ...
    public function line_entry_data($opname = 'products', $is_text = 0,$mod = '') {
        ...
        if ($this->get_id() != '') {
            ...
            $query = $this->Salesorder->select_one(array('_id' => new MongoId($this->get_id())),array('options','products','sum_sub_total','sum_amount','sum_tax','rfqs','salesorder_date'));
                ...
                foreach ($query[$opname] as $key => $arr) {
                    if (!$arr['deleted']) {
                        ...
                        if(!isset($arr['option_for'])){
                            $option = $this->new_option_data(array('key'=>$key,'products_id'=>$arr['products_id'],'options'=>$query['options'],'products'=>$query['products'],'date'=>$query['salesorder_date']),$query['products']);
						...
						$newdata[$key]['_id'] = $key;
						$newdata[$key]['sort_key'] = $this->Salesorder->num_to_string($key).'-'.'0';
						...
						if(isset($arr['option_for']) && $arr['option_for']!=''){
							...
							$newdata[$key]['sort_key'] = $this->Salesorder->num_to_string($arr['option_for']).'-'.$key;
							if($mod!='options_list')
            ...
            if(isset($option_for_sort['parent'])){
                foreach($option_for_sort['parent'] as $p_key=>$parent){
                    ...
                    if(isset($option_for_sort['option'])){
                        ...
                        $option_for_sort['option'][$p_key] = $this->Salesorder->aasort($option_for_sort['option'][$p_key],'option_group_for_sort');
    ...
    function line_entry(){
    	...
    	$this->selectModel('Salesorder');
            //get entry data
    	$ids = $this->get_id();
        $date_modified = $this->Salesorder->select_one(array('_id'=>new MongoId($ids)),array('date_modified','status'));
        ...
        if(isset($date_modified['status']) && $date_modified['status'] == 'Cancelled' )
    ...
    public function line_entry_add(){
    	$this->selectModel('Salesorder');
    	$quote = $this->Salesorder->select_one(array('_id' => new MongoId($this->get_id())),array('products'));
    ...
    public function option_list($idsub) {
        ...
        $query = $this->Salesorder->select_one(array('_id'=> new MongoId($this->get_id())),array('products','options','salesorder_date'));
        if(!isset($query['options']))
            $query['options'] = array();
        ...
        //DATA: option list
        $option_list_data = $this->new_option_data(array('key'=>$idsub,'products_id'=>$products_id,'options'=>$query['options'],'date'=>$query['salesorder_date']),$query['products']);

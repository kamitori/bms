...
function EventRightClick() {
	...
	this.MouseMenu = {
		...
		display: function(e, button) {
			...
			if (button === "RIGHT") {
				...
				mouseMenu.style.left = posx+1 + "px";

				// BaoNam
				// lấy ra div có id để lấy id của event
				var element_div = e.toElement;
				if( e.toElement == undefined ){
					element_div = e.target;
				}
				var div = $(element_div);
				...
				if( div.attr("id") == undefined ){
					...
					if( div.attr("id") == undefined ){
						...
						if( div.attr("id") == undefined ){
							...
							alert("Id div not defined, please contact developer. Error: element_div = e.target, line 79 rightclick.js" );
						}
					}
				}
				$("#mouse_menu_right").attr("rel", div.attr("id"));

				// BaoNam
				if( $("#dhtmlXTooltip_tooltip_css").attr("id") != undefined ){
					$("#dhtmlXTooltip_tooltip_css").html('<style type="text/css">.dhtmlXTooltip.tooltip{display:none !important;}</style>');
				}
	...
	this.MouseButton = {
		targetContainer: document.getElementById("click_area"),
		...
		init: function() {
			$(".rightclick").each(function(){
				thisObject.addEvent(document.getElementById($(this).attr("id")), "mouseover", thisObject.MouseButton.start);
				thisObject.addEvent(document.getElementById($(this).attr("id")), "mouseout", thisObject.MouseButton.end);
			});
		...
		/* End */
			...
			while (reltg != undefined && reltg != tg && reltg.nodeName != "BODY") {
...
$(function(){
	$(document).on("click", function(e){
		...
		if( button == "LEFT" && $("#mouse_menu_right").css("visibility") != "hidden" ){

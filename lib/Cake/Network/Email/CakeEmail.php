 ...
 */
	public function config($config = null, $config_more = null) {
		if ($config === null) {
			return $this->_config;
		}
		if (!is_array($config)) {
		...
		$this->_applyConfig($config, $config_more);
		return $this;
	}
 ...
 *   the named config.
 */
	protected function _applyConfig($config, $config_more = null) {
		if (is_string($config)) {
		...
		// BaoNam 20/09
		if( is_array($config_more) )
			$config = array_merge($config, $config_more);

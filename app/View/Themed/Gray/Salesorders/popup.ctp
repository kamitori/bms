                ...
                <div class="box_inner_search float_left">
                    ...
                    <div class="styled_select2 float_left">
                        ...
                        echo $this->Form->input($model.'.code', array(
                            'id' => 'window_popup_'.$controller.'_Code_' . $key,
                            'style' => 'background: #636363;color: #fff;margin-left: -2px;margin-top:-2px;'
                        ));
                        ?>
                    </div>
                    <a href="javascript:void(0)" onclick="$('#window_popup_<?php echo $controller; ?>_Code_<?php echo $key; ?>').val('');
...
<div class="block_dent2 container_same_category" style="overflow: auto;overflow-x: hidden;max-width:1000px; margin: 0 auto; height:430px;" id="list_view_quotations">
    <table class="jt_tb" id="Form_add" style="font-size:12px;">
        <thead style="position: fixed;" id="pagination_sort">
            <tr>
                <th style="width:60px"><?php echo translate('Ref no'); ?><span id="sort_code" rel="code" class="desc"></span></th>
        ...
        <tbody>
            ...
            foreach ($arr_salesorders as $value) {
                $i = 1 - $i; $STT += 1;
                ?>
                <tr class="jt_line_<?php if ($i == 1) { ?>black<?php } else { ?>light<?php } ?>" onclick="after_choose_salesorders<?php if (substr($key, 0, 1) == '_') echo $key; ?>('<?php echo $value['_id']; ?>', '', '<?php echo $key; ?>')">
                    ...
                    <td align="center"><?php if(isset($value['salesorder_date']))echo $this->Common->format_date( $value['salesorder_date']->sec, false); ?>
                        <input type="hidden" id="after_choose_salesorders<?php echo $key; ?><?php echo $value['_id']; ?>" value="<?php echo htmlentities(json_encode($value)); ?>">
                    </td>
                </tr>
            <?php } ?>

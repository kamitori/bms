...
class SalesordersController extends AppController {
	...
	public function beforeFilter() {
		...
		$this->set_module_before_filter('Salesorder');
			...
			if(isset($query['job_id'])&&is_object($query['job_id'])){
                ...
                if($continue && isset($job['status']) && $job['status'] == 'Completed'){                	
                    if($valid){
                    	...
                    	//Nếu có có quyền này thì không lock
                    	if(!$this->check_permission($this->name.'_@_change_status_job_completed_@_edit')){
                    		$this->opm->set_lock(array('name'), 'out');	
                    	}                        
                        $this->set('address_lock', '1');
                    }
	...
	function ajax_save(){
		if(!$this->check_cond_process_payment()){
			...
			if( $_POST['field'] == 'code') {
				...
				$id = $this->get_id();
				$query = $this->opm->select_one(array('_id'=> new MongoId($id)),array('status', 'job_id', 'code', 'contact_id', 'contact_name', 'email'));
				if($query['status'] == 'Completed' && !$this->check_permission($this->name.'_@_change_status_@_edit')){
					...
					//Không cancel job vì một ngày chỉ có một job dùng cho nhiều SO
					/*if( isset($query['job_id']) && is_object($query['job_id']) ) {
						$this->selectModel('Job');
						$this->Job->save(array('_id' => $query['job_id'], 'status' => 'Cancelled', 'status_id' => 'Cancelled'));
					}*/
				} else if($_POST['value'] == 'Returned'){
					//update and send email to customer
					$this->opm->save(array('_id' => new MongoId($id), 'status' => 'Returned', 'asset_status' => 'Returned', 'send_mail_return' => 1, 'return_paid' => 1 ));
					$this->sendmail_customer('Returned', $query);
				}
	...
	function sendmail_customer($case, $order){
		if (!isset($order['email']) || !filter_var($order['email'], FILTER_VALIDATE_EMAIL)) {
			return 'invalid email';
		}
		if($case == 'Returned'){
			$subject = 'Banhmisub - Sales Order Ref No: '.$order['code'];
			$email = array(
					   'contact_name' 	=> 	$order['contact_name'],
					   'contact_id'		=>	'',
					   'content'		=> 	'Dear '.$order['contact_name'].', your order is returned. Thanks.',
					   'template'		=> 	'Dear '.$order['contact_name'].', your order is returned. Thanks.'
					   );
			$email = array_merge($email, $order);
	...
	function create_full_salesinvoice($saleorder_id=''){
		...
		{
			$ids = $saleorder_id;
			$combine_from_job = 'combine_from_job';
		}
		...
		if(isset($arr_salesorder['salesinvoice_id'])&&is_object($arr_salesorder['salesinvoice_id'])){
			...
			if(isset($salesinvoice['invoice_status']) && $salesinvoice['invoice_status'] != 'Cancelled'){
				...
				echo URL . '/salesinvoices/create_salesinvoice_from_salesorder/'.$ids.'/'.$combine_from_job;
				die;
			}
		}
		else{
...
// Popup form orther module
	function popup($key = "") {
        ...
        if (is_object($arr_salesorders)) {
            ...
            $total_record = $this->Salesorder->count($cond);
            if( $total_record%$limit != 0 ){
                $total_page = floor($total_record/$limit) + 1;
            }else{
                $total_page = $total_record/$limit;
	...
	function deleteAllTasks($salesorder_id) {
		$this->selectModel('Task');
		$arr_task = $this->Task->select_all(array(
			'arr_where' => array('salesorder_id' => new MongoId($salesorder_id))
		));
		if($arr_task) {
			foreach ($arr_task as $key => $value) {
				$this->tasks_delete_execute((string)$value['_id']);
			}
		}
	}
	...
	public function _get_default($salesorder_id){
		$this->selectModel('Salesorder');
		$arr_salesorder = $this->Salesorder->select_one(array('_id' => new MongoId($salesorder_id)),array('created_by','name','payment_due_date'));
		$arr_save = array();
		$arr_save['salesorder_id'] = new MongoId( $salesorder_id );
	...
	function _create_task_belongto_product($salesorder_id){
		...
		foreach($arr_asset as $asset_id => $asset){
			...
			if( isset($arr_tmp['_id']) ){
				...
				$arr_save_task = $arr_tmp;
				$arr_save_task['production_time'] = $asset['production_time']; // cập nhật production_time mới
				// +++++++++++++++++
				$arr_save_task['work_end'] = $arr_save['work_end'];
				$arr_save_task['work_start'] = new MongoDate( $arr_save_task['work_end']->sec - $asset['production_time']*3600 ); // cập nhật thời gian mới khác	
				
				if (!$this->Task->save($arr_save_task)){
	...
	function tasks_auto_save($field = '', $force_update = 0) {
		...
		$so = $this->Salesorder->select_one(array('_id' => new MongoId($arr_save['salesorder_id'])), array('_id','name', 'salesorder_date', 'payment_due_date','status','payment_terms','job_id','asset_tags'));
		...
		if($field == 'status' && $arr_save['status'] == 'DONE'){
			...
			if(isset($so['asset_tags']) && count($so['asset_tags'])>0 && isset($arr_save['_id'])){
				...
				if(isset($arr_save['salesorder_id'])){
					...
					$this->Salesorder->save($arr_update_asset);
				}
	...
	function tasks_add($salesorder_id, $our_rep_type = 'contacts', $rep_id, $rep) {
		$result = $this->tasks_add_execute($salesorder_id, $our_rep_type, $rep_id, $rep);
		if ($result) {
	...
	function tasks_add_execute($salesorder_id, $our_rep_type = 'contacts', $rep_id, $rep) {
		...
		$this->selectModel('Task');
		$this->Task->arr_default_before_save = $arr_save;
		if ($id = $this->Task->add()) return true;
		return false;
	}

	function tasks_delete($id) {
		
		if ($this->tasks_delete_execute($id))
			echo 'ok';
		else
			echo 'Error: ' . $this->Task->arr_errors_save[1];
		die;
	}
	function tasks_delete_execute($id) {
		...
		$this->selectModel('Task');
		if ($this->Task->save($arr_save)) return true;
		return false;
	}
	...
	public function calendar_json($type = 'contacts', $date_from = '', $date_to = '', $get_more = 'no'){
		...
		foreach ($arr_tasks as $value) {
			if (isset($value['salesorder_id']) && is_object($value['salesorder_id'])) {
				$tmp_salesorder = $this->Salesorder->select_one(array('_id' => $value['salesorder_id']), array('_id', 'name', 'our_rep', 'our_csr'));
	...
	public function create_sale_order($id = '') {
		...
		if ($quotation) {
			...
			$this->Salesorder->arr_default_before_save = $arr_default_before_save;
			...
			if ($this->Salesorder->add()) {
				...
				$return_id = $this->Salesorder->mongo_id_after_save;
				// update quotation.
	...
	public function customer_report($type = ''){
        ...
        if(isset($_GET['print_pdf'])){
			...
			if(isset($_POST) && !empty($_POST)){
				...
				/*$salesorder = $this->Salesorder->select_all(array(
						'arr_where'=>$arr_where,
						'arr_order'=>array(
										'sum_sub_total'=>-1
										),
                ...
                $count = $this->Salesorder->count($arr_where,array('limit' => 9999999));
				if($count == 0) {
					echo 'empty';
					die;
				} else {
	...
	public function check_condition_salesorder()
	{
		$this->selectModel('Salesorder');
		$id = $this->get_id();
		$salesorder = $this->Salesorder->select_one(array('_id' => new MongoId($id)));
		if($salesorder!='')
		{
	...
	public function check_condition_back_order($type = 'invoice')
	{
		...
		$back_order = $this->Salesorder->select_all(array(
											'arr_where'=>$arr_where,
	...
	public function receive_item() {
		...
		$salesorder_id = $this->get_id();
		$this->selectModel('Salesorder');
		$arr_salesorders = $this->Salesorder->select_one(array('_id' => new MongoId($salesorder_id)),array('products'));
	...
	*/
    ...
    function fix_no_one_order($order_id,$new_no){
    	if($order_id!='' && $new_no!=''){
	    	$this->opm->collection->update(array('_id' => new MongoId($order_id)), array('$set'=> array('code' => $new_no)), array("multiple" => true));
	    	echo 'Done'; die;
    	}
    }
    function resort_number_order($str_date = '17-04'){
    	$orders = $this->opm->select_all(array(
	    	                               'arr_where'  =>  array (
	    	                               			"code"=> new MongoRegex("/".$str_date."/i")
	    	                               					),
    	...
    	$no = 1;
    	foreach ($orders as $key => $value) {
    		$new_code = $str_date.'-'.str_pad($no, 4, "0", STR_PAD_LEFT);
    ...
    function move_monthorder_to_order(){
    	$this->selectModel('Somonth');
    	$somonth = $this->Somonth->select_all(array(		'arr_where'  =>  array (),
					                            'arr_order'	=> array ('_id' => 1)
					                       ));
    	$newcode = $this->opm->get_auto_code('code');
    	$arr_tmp = explode("-", $newcode);
    	$num = end($arr_tmp);
    	$newnum = str_pad((int)$num+1, 4, 0, STR_PAD_LEFT);
    	// pr($newnum);
    	$arr_save = array();
    	$arr_save['type'] = 'salesorder_month';
    	$arr_save['salesorder_id'] = '';
    	foreach ($somonth as $key => $value){
    		$value['code'] = date("y-m").'-'.str_pad((int)$num+1, 4, 0, STR_PAD_LEFT);
    		echo $value['code'].'<br >';
    		$num++;
    		$this->opm->collection->insert($value, array('safe'=>true));
    		$arr_save['salesorder_id'] = (string)$value['_id'];
    	}
    ...
    function next_move_month_order(){
    	$this->selectModel('Stuffs');
    	$old_so = $this->Stuffs->select_one(array('type'=>"salesorder_month"));
    	// $move_date = $old_so['date_modified'];
    	$this->selectModel('Somonth');
    	$somonth = $this->Somonth->select_all(array('arr_where'  =>  array ('_id'=>array('$gt'=>new MongoId($old_so['salesorder_id']))),
					                            'arr_order'	=> array ('_id' => 1)
					                       ));
    	// foreach ($somonth as $key => $value){
    	// 	echo $value['code'].'<br >';

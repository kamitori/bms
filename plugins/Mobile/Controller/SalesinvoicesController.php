<?php
class SalesinvoicesController extends MobileAppController {
	...
	public function entry($id = '0', $num_position = -1) {
		$arr_tmp = $this->entry_init($id, $num_position, 'Salesinvoice', 'salesinvoices');
		$arr_tmp['invoice_date'] = (is_object($arr_tmp['invoice_date'])) ? date('m/d/Y', $arr_tmp['invoice_date']->sec) : '';
		$arr_tmp['payment_due_date'] = (is_object($arr_tmp['payment_due_date'])) ? date('m/d/Y', $arr_tmp['payment_due_date']->sec) : '';
		if (isset($arr_tmp['paid_date']))
			$arr_tmp['paid_date'] = (is_object($arr_tmp['paid_date'])) ? date('m/d/Y', $arr_tmp['paid_date']->sec) : '';
		else
			$arr_tmp['paid_date'] = '';

		$this->selectModel('Setting');
		$arr_salesinvoices_type = $this->Setting->select_option(array('setting_value' => 'salesinvoices_type'), array('option'));
		$this->set('arr_salesinvoices_type', $arr_salesinvoices_type);

		$arr_salesinvoices_payment_terms = $this->Setting->select_option(array('setting_value' => 'salesinvoices_payment_terms'), array('option'));
		$this->set('arr_salesinvoices_payment_terms', $arr_salesinvoices_payment_terms);

		$arr_salesinvoices_status = $this->Setting->select_option(array('setting_value' => 'salesinvoices_status'), array('option'));
		$this->set('arr_salesinvoices_status', $arr_salesinvoices_status);

		$this->selectModel('Tax');
		$arr_salesinvoices_taxtext = $this->Tax->tax_select_list();
		//$arr_salesinvoices_taxtext = $this->Setting->select_option(array('setting_value' => 'product_pst_tax'), array('option'));
		$this->set('arr_salesinvoices_taxtext', $arr_salesinvoices_taxtext);	
		...
		$arr_tmp1['Salesinvoice'] = $arr_tmp;
		$this->data = $arr_tmp1;
		$arr_salesinvoice_id = array();
		if (isset($arr_tmp['our_rep_id']))
			$arr_salesinvoice_id[] = $arr_tmp['our_rep_id'];
	...
	function auto_save() {
		if (!empty($this->data)) {
			$id = $this->get_id();
			$arr_return = array();
			$arr_post_data = $this->data['Salesinvoice'];
			...
			if(isset($arr_save['code'])){
				...
				$arr_tmp = $this->Salesinvoice->select_one(array('code' => $arr_save['code'], '_id' => array('$ne' => new MongoId($id))));
			...
			$arr_tmp = $this->Salesinvoice->select_one(array('_id' =>  new MongoId($id)),array('salesinvoice_status','company_id','customer_po_no'));

			$invoice_date = $this->Common->strtotime($arr_save['invoice_date'] . ' 00:00:00');
			$arr_save['invoice_date'] = new MongoDate($invoice_date);

			$paid_date = $this->Common->strtotime($arr_save['paid_date'] . ' 00:00:00');
			$arr_save['paid_date'] = new MongoDate($paid_date);
			...
			if(strlen($arr_save['company_id'])==24){
				...
				if(isset($arr_tmp['company_id'])
					...
					$arr_save['name'] = (string)$arr_save['code'].'-'.$arr_save['company_name'];
				}
			}
			else
				$arr_save['company_id'] = '';
			...
			if ($this->Salesinvoice->save($arr_save))
				...
				$arr_return = array('status'=>'error','message'=>'Error: ' . $this->Salesinvoice->arr_errors_save[1]);
	...
	function lists( $content_only = '' ) {
		...
		// dùng cho điều kiện
		$cond = array();
		if( $this->Session->check('salesinvoices_entry_search_cond') ){
			$cond = $this->Session->read('salesinvoices_entry_search_cond');
		}
		...
		$this->selectModel('Setting');
		$this->set('arr_salesinvoices_type', $this->Setting->select_option(array('setting_value' => 'salesinvoices_type'), array('option')));
		...
		if (!$error) {
			...
			if ($this->Salesinvoice->save($arr_save)) {
				...
				if($this->request->is('ajax'))
					...
					$this->redirect('/mobile/salesinvoices/entry');
			} else {
	...
	public function add() {
		...
		if ($this->Salesinvoice->add())
			$this->redirect('/mobile/salesinvoices/entry/' . $this->Salesinvoice->mongo_id_after_save);
		die;
	}
	...
	function popup($key = "") {
        ...
        if (isset($_POST['sort']) && strlen($_POST['sort']['field']) > 0) {
            ...
            $arr_salesinvoice = array($_POST['sort']['field'] => $sort_type);
        ...
        $arr_salesinvoices = $this->Salesinvoice->select_all(array(
            'arr_where' => $cond,
            'arr_salesinvoice' => $arr_order,
            'limit' => $limit,
            'skip' => $skip,
            'arr_field' => array('company_id', 'company_name', 'contact_name','invoice_date','name','code')
        ));
	...
	public function line_entry_data($opname = 'products', $is_text = 0,$mod = '') {
        ...
        if ($this->get_id() != '') {
            ...
            $query = $this->Salesinvoice->select_one(array('_id' => new MongoId($this->get_id())),array('options','products','sum_sub_total','sum_amount','sum_tax','invoice_date'));
            ...
            if (isset($query[$opname]) && is_array($query[$opname])) {
                ...
                foreach ($query[$opname] as $key => $arr) {
                    if (!$arr['deleted']) {
						...
						$newdata[$key]['_id'] = $key;
						$newdata[$key]['sort_key'] = $this->Salesinvoice->num_to_string($key).'-'.'0';
						...
						if(isset($arr['option_for']) && $arr['option_for']!=''){
							...
							$newdata[$key]['sort_key'] = $this->Salesinvoice->num_to_string($arr['option_for']).'-'.$key;
							if($mod!='options_list')
            ...
            if(isset($option_for_sort['parent'])){
                foreach($option_for_sort['parent'] as $p_key=>$parent){
                    ...
                    if(isset($option_for_sort['option'])){
                        ...
                        $option_for_sort['option'][$p_key] = $this->Salesinvoice->aasort($option_for_sort['option'][$p_key],'option_group_for_sort');
    ...
    function line_entry(){
    	...
    	$this->selectModel('Salesinvoice');
            //get entry data
    	$ids = $this->get_id();
        $date_modified = $this->Salesinvoice->select_one(array('_id'=>new MongoId($ids)),array('date_modified','invoice_status'));
        ...
        if(isset($date_modified['invoice_status']) && $date_modified['invoice_status'] == 'Cancelled' )
    ...
    public function line_entry_add(){
    	$this->selectModel('Salesinvoice');
    	$quote = $this->Salesinvoice->select_one(array('_id' => new MongoId($this->get_id())),array('products','currency'));
    	...
    	if($field == 'products'){
    		if( isset($options['key']) && !empty($value['products'][$options['key']]) ){
    			if(isset($value['products'][$options['key']]['deleted']) && $value['products'][$options['key']]['deleted']){
	                ...
	                $value['products'][$options['key']]['sell_price'] = isset($parent['sell_price'])?$parent['sell_price']:0;
	                $value['products'][$options['key']]['unit_price'] = isset($parent['unit_price'])?$parent['unit_price']:0;
	                $value['products'][$options['key']]['custom_unit_price'] = isset($parent['unit_price'])?$parent['unit_price']:0;
	                ...
	                $value['products'][$options['key']]['is_custom_size'] = isset($parent['is_custom_size'])?$parent['is_custom_size']:0;
	                ...
	                $value['products'][$options['key']]['receipts'] = '';
	                $value['products'][$options['key']]['option'] = '';
	                $value['products'][$options['key']]['products_costing_name'] = '';
	                $value['products'][$options['key']]['oum_depend'] = isset($parent['oum_depend'])?$parent['oum_depend']:'';
	                $value['products'][$options['key']]['sub_total'] = isset($parent['sell_price'])?$parent['sell_price']:'';
	                $value['products'][$options['key']]['plus_sell_price'] = 0;
	                $value['products'][$options['key']]['plus_unit_price'] = 0;
	                $value['products'][$options['key']]['amount'] = $value['products'][$options['key']]['sell_price'] * (1 + $value['products'][$options['key']]['taxper']/100 );
    ...
    public function option_list($idsub) {
        ...
        //neu idopt khac rong
        //DATA: salesinvoice line details
        $query = $this->Salesinvoice->select_one(array('_id'=> new MongoId($this->get_id())),array('products','options','invoice_date'));
        if(!isset($query['options']))
            $query['options'] = array();
        ...
        //DATA: option list
        $option_list_data = $this->new_option_data(array('key'=>$idsub,'products_id'=>$products_id,'options'=>$query['options'],'date'=>$query['invoice_date']),$query['products']);
        ...
        foreach($receipts as $value)
            foreach($value['allocation'] as $key=>$val){
                ...
                if($val['salesinvoice_id']==$id && !$val['deleted'] ){
                    ...
                    $arr_receipt[$value['code']]['date'] =  date('d M, Y', $value['receipt_date']->sec);
